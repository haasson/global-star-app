   ...
   <form class="form" v-if="status === 'new'" @submit.prevent="sendEmail">
      ...
      <div class="form-inner">
         ...
         <div class="agreement" @click="toggleAgreement">
            <AppIcon class="icon-check" :name="formData.agreement ? 'checkbox' : 'check'"/>
            <p>
         ...
         <div v-if="hasError && showError" class="errors">
            ...
            <p v-if="!formData.agreement">Вы должны согласиться на обработку персональных данных</p>
         </div>
...
export default {
   ...
   setup(props, {emit}) {
      ...
      const hasError = computed(() => {
         return (!formData.name || !formData.phone || formData.phone.length < 18 || !formData.agreement) && status.value === 'new'
      })
